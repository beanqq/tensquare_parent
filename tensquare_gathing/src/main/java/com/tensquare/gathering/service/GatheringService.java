...
public class GatheringService {
	...
	/**
	 * 根据ID查询实体
	 * 运用spring chache缓存数据    spring chache是方法级别的缓存，针对方法的返回结果进行缓存
	 * 并把方法参数和方法结果，以Key-value键值对的方式存到缓存中
	 * 不可以设定过期时间
	 * @param id
	 * @return
	 */
	@Cacheable(value = "gathering",key = "#id")  //value代表意思是:缓存的表示  key:缓存的id   #id可以获取参数的id
